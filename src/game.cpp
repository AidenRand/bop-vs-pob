	...
	while (window.isOpen())
	{
		...
		else
		{
			...
			for (long unsigned int i = 0; i < p1_proj_vector.size(); i++)
			{
				...
				player_1.strongAttackCollision(player_2, p1_weak_attack, p1_proj_vector[i]);
			...
			for (long unsigned int i = 0; i < p2_proj_vector.size(); i++)
			{
				...
				player_2.strongAttackCollision(player_1, p1_hit_status, p2_proj_vector[i]);
			...
			player_1.weakAttackCollision(player_2, p1_weak_attack, p2_hit_status);
			...
			player_2.weakAttackCollision(player_1, p2_weak_attack, p1_hit_status);
